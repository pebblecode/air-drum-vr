 ...
 *
 * Modified by Peter O'Shaughnessy to allow a separate overlay scene (for our text)
 */
THREE.VREffect = function ( renderer, options ) {
  ...
  options || (options = {})
    ...
    // Modified by Peter to take an overlay scene
    // See: http://stackoverflow.com/questions/13309289/three-js-geometry-in-top-of-another
	this.render = function ( scene, camera, overlayScene ) {
        ...
        // Modified by Peter to include overlay scene
        renderer.clear();
		renderer.render.apply( this._renderer, [scene, camera] );
        renderer.clearDepth();
        if( overlayScene ) {
            renderer.render.apply( this._renderer, [overlayScene, camera] );
        }
	};

    // Modified by Peter to take an overlay scene
    // See: http://stackoverflow.com/questions/13309289/three-js-geometry-in-top-of-another
	this.renderStereo = function( scene, camera, overlayScene, renderTarget, forceClear ) {
        ...
        renderer.clearDepth();
        if( overlayScene ) {
            renderer.render( overlayScene, cameraLeft );
        }
        ...
        renderer.clearDepth();
        if( overlayScene ) {
            renderer.render( overlayScene, cameraRight );
        }
